...
class AnchorsGenerator(nn.Module):
    __annotations__ = { "cell_anchors": Optional[List[torch.Tensor]],
...
class RPNHead(nn.Module):
    ...
    通过RPNHead计算预测目标概率与bbox regression参数
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head, fg_iou_thresh, bg_iou_thresh, batch_size_per_image,
                 positive_fraction, pre_nms_top_n, post_nms_top_n, nms_thresh, score_thresh=0.0):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # 计算边界框回归损失
        box_loss = det_utils.smooth_l1_loss(pred_bbox_deltas[sampled_pos_inds],
    ...
    def forward(self,
        ...
        # objectness和pred_bbox_deltas都是list / objectness对象类别 pred_bbox_deltas bbox预测
