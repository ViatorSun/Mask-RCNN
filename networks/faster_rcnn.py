...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        features = self.backbone(images.tensors)            # 将图像输入backbone得到 特征金字塔
        ...
        proposals, proposal_losses  = self.rpn(images, features, targets)    # rpn_function.RegionProposalNetwork
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if box_roi_pool is None:
            box_roi_pool = MultiScaleRoIAlign(  featmap_names=['0', '1', '2', '3'],     # 在哪些特征层进行roi pooling
                                                output_size=[7, 7],
                                                sampling_ratio=2)
