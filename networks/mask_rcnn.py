...
class MaskRCNN(FasterRCNN):
    ...
    def __init__( self, backbone, num_classes=None,
            ...
            box_nms_thresh=0.5, box_detections_per_img=100, box_fg_iou_thresh=0.5, box_bg_iou_thresh=0.5,
        ...
        super().__init__( backbone, num_classes,
            ...
            rpn_post_nms_top_n_train, rpn_post_nms_top_n_test, rpn_nms_thresh, rpn_fg_iou_thresh,
            rpn_bg_iou_thresh, rpn_batch_size_per_image, rpn_positive_fraction, rpn_score_thresh,

            # Box parameters
            box_roi_pool, box_head, box_predictor, box_score_thresh, box_nms_thresh,
            box_detections_per_img, box_fg_iou_thresh, box_bg_iou_thresh,
            box_batch_size_per_image, box_positive_fraction, bbox_reg_weights    )

        self.roi_heads.mask_roi_pool  = mask_roi_pool       # MultiScaleRoIAlign
        self.roi_heads.mask_head      = mask_head           # MaskRCNNHeads
        self.roi_heads.mask_predictor = mask_predictor      # MaskRCNNPredictor
...
class MaskRCNNHeads(nn.Sequential):
    def __init__(self, in_channels, layers, dilation):
        ...
        for layer_idx, layers_features in enumerate(layers, 1):
            d[f"mask_fcn{layer_idx}"] = nn.Conv2d(next_feature, layers_features, 3, 1, padding=dilation, dilation=dilation)
...
class MaskRCNNPredictor(nn.Sequential):
    def __init__(self, in_channels, dim_reduced, num_classes):
        super().__init__(OrderedDict([  ("conv5_mask", nn.ConvTranspose2d(in_channels, dim_reduced, 2, 2, 0)),
