...
* clone from：https://github.com/pytorch/vision/tree/master/references/detection
...
```
  ├── backbone: 特征提取网络
  ├── network: Mask R-CNN网络
  ├── train_utils: 训练验证相关模块（包括coco验证相关）
  ├── dataset_coco.py: 自定义dataset用于读取COCO2017数据集
  ├── dataset_voc.py: 自定义dataset用于读取Pascal VOC数据集
